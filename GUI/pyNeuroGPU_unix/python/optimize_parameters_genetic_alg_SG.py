...
def main():
    ...
    #
    if (cp_true ==True):
        pop, hof, log, hst = opt.run(max_ngen=1, cp_filename=cp_file, cp_frequency=1,continue_cp=True)
    else:
        pop, hof, log, hst = opt.run(max_ngen=1, cp_filename='cp.pkl', cp_frequency=1)
