...
params_dir =        '../Data'                                        # params directory (folder)

params_file =       f'./{params_dir}/orig1.csv'                    # 12 parameters
...
def assemble_allparams(allparams_template, params, param_to_allparam, param_mappings, model_mappings):
    ...
    mylog.append('Starting assemble_allparams ***1')
    mylog.append('printing allparams_template')
    mylog.extend(allparams_template)
    mylog.append('printing params')
    mylog.extend(params)
    mylog.append('printing param_to_allparam')
    mylog.extend(param_to_allparam)
    mylog.append('printing param_mappings')
    mylog.extend(param_mappings)
    mylog.append('printing model_mappings')
    mylog.extend(model_mappings)
    
    
    
    global reversed_mappings
    model_mappings_flat = flatten_dict(model_mappings)
    mylog.append('printing model_mappings_flat')
    mylog.extend(model_mappings_flat)
    param_mappings_flat = flatten_dict(param_mappings)
    mylog.append('printing param_mappings_flat')
    mylog.extend(param_mappings_flat)
    ...
    with open(f'{data_dir}/biophysics_mapping.json', 'r') as bph:
        bphm = load(bph)
    #-----------------------------------------------------------------------------------------------------
    mylog.append('printing biophysics_mapping')
    mylog.extend(bphm)
    ...
    mylog.append('printing index_map')
    mylog.extend(index_map)
    # create the template to fill with values
    mylog.append('printing allparams_template **2')
    mylog.extend(allparams_template)
    mylog.append('printing params **2')
    mylog.extend(params)
    mystr=''
    mystr = '\n'.join(map(str,mylog))
    ...
    mylog.append('printing template0')
    mylog.extend(template)
    mystr='\n'
    mystr = '\n'.join(map(str,mylog))
    lf.write(mystr)
    ...
    mylog.clear()
    mylog.append('printing template1')
    mylog.extend(template)
    mystr='\n'
    mystr = '\n'.join(map(str,mylog))
    lf.write(mystr)
    ...
    mylog.clear()
    mylog.append('printing template2')
    mylog.extend(template)
    mystr='\n'
    mystr = '\n'.join(map(str,mylog))
    lf.write(mystr)
        ...
        with open(fl) as jf:
            ...
            for i in range(len(params)):
                for k, v in dct.items():
                    template[i][int(k)] = v
    mylog.clear()
    mylog.append('printing template3')
    mylog.extend(template)
    mystr='\n'
    mystr = '\n'.join(map(str,mylog))
    lf.write(mystr)
...
def allparams_from_mapping(params_input=None):
    ...
    if params_input is None:
        with open(params_file, 'r') as p:
            ...
            params = params_input
    mylog.append('printing params_input')
    mylog.extend(params)
    # get all param mappings from json
    with open(param_map, 'r') as pm:
        param_mappings = load(pm)
    mylog.append('printing param_map')
    mylog.extend(param_map)
