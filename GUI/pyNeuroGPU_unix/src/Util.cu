...
MYFTYPE * ReadArrayFromFile(const char* FN, int *ans ) {
    ...
    MYFTYPE* data_out = (MYFTYPE*)malloc(sizeof(double)*dims_out[0]*dims_out[1]);
    if (data_out == NULL) {
        printf("Malloc Error in ReadArrayFromFile");
        throw "Malloc Error in ReadArrayFromFile";
    }
    dataset.read( data_out, PredType::NATIVE_FLOAT, H5S_ALL, H5S_ALL );
    ...
    //open set
    const H5std_string  H5_Nam_Height("nSets");
    DataSet             H5_Set_Height = file.openDataSet(H5_Nam_Height);

    //read set
    H5_Set_Height.read(ans, PredType::NATIVE_INT, H5S_ALL, H5S_ALL);
