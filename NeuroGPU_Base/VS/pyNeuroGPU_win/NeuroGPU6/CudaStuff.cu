...
#define init_state_macro(stateInd,segmentInd) InitStatesM[NeuronID*perThreadStateMSize + stateInd*InMat.NComps+cSegToComp[segmentInd] ]	;
	...
	for (int count = 1; count < NILP + 1; count++) {
	if(cBoolModel[PIdx[count] +0*NSEG]){CuInitModel_pas(v[count],param_macro(0, PIdx[count]) ,param_macro(1, PIdx[count]) );}
	}
	...
	for (int i = 0; i<Nt; i++) {
		...
		for (int count = 1; count < NILP + 1; count++) {
		   if(cBoolModel[PIdx[count] +0*NSEG]){CuBreakpointModel_pas(sumCurrentsDv[count] , sumConductivityDv[count] ,v[count] +0.001,param_macro(0, PIdx[count]) ,param_macro(1, PIdx[count]) );}
		  if(cBoolModel[PIdx[count] +0*NSEG]){CuBreakpointModel_pas(sumCurrents[count] , sumConductivity[count],v[count] ,param_macro(0, PIdx[count]) ,param_macro(1, PIdx[count]) );}
...
for (int count = 1; count < NILP + 1; count++) {
	  if(cBoolModel[PIdx[count] +0*NSEG]){}
		}
