...
def prun():    
    ...
    h.tstop = tstop
    pc.psolve(tstop)
coreneuron.enable = cn_flg
coreneuron.gpu = cn_flg
...
np.savetxt(f'vs_{ncell}.txt',all_volts)

pc.done()
with open(f'times_{ncell}.txt', "w") as myfile:
    myfile.write(f'it took {total_time}')
h.quit()
