...
def nrnMread(fileName):
    ...
    file = np.fromfile(f,np.double).astype(dtype='float16')    
    f.close()
    return file
...
def check_ap_at_zero(stim_ind,indvs):
    """
    ...
    i = 0
    with open('/tmp/Data/VHotP' + str(stim_ind) + '.dat', 'rb') as f:
        for voltset in read_in_chunks(f,stim_ind):
            volt = voltset
            if first_zero_ind:
...
#         print(1/0)
    ...
    def eval_stim_sf_pair(self,perm):
        ...
        curr_data_volt = [] # change later?
        #print(curr_data_volt.shape, "CURR DATA VOLTs")
    ...
    def map_par(self):
        ...
        #res = client.map(self.eval_stim_sf_pair, fxnsNStims)
        #wait(res) # maybe remove later?
        #print(res)
        #res = [f.result() for f in res]
        #res = np.array(res) 
