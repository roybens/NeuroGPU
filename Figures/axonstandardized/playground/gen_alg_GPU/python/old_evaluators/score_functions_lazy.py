...
#if this works, just import it
def read_in_chunks(f, idx,chunk_size=80000):
    """Lazy function (generator) to read a file piece by piece.
    Default chunk size: 1k."""
    #file = "../Data/VHotP" + str(idx) + ".dat"
    nparam = struct.unpack('i', f.read(4))[0]
    typeFlg = struct.unpack('i', f.read(4))[0]
    if os.path.exists("../Data/VHotP" + str(idx) + ".dat"): 
        size = os.stat("../Data/VHotP" + str(idx) + ".dat").st_size
    float_format = array('d')
    while True:
        data = None # throw out old data, this is the whole point to save overhead
        ...
        if not data:
            break
        float_format.fromstring(data)
        s = struct_unpack(data)
        #print(len(s), "struct len")
        #print(len(float_format), "len float format")
        yield s
...
def eval_efel(feature_name, target, idx, dt=0.02, stims=None, index=None):
    ...
    test_counter = 0
    with open('/tmp/Data/VHotP' + str(idx) + '.dat', 'rb') as f:
        for voltset in read_in_chunks(f,idx): # use this for customsfs and for setting up efel 
            curr_trace_data = {}
            curr_trace_data['T'] = time
            nan_inds_bol = np.isnan(voltset)
            if nan_inds_bol.any():
                print("there are nan values")
                print(1/0)
            curr_trace_data['V'] = voltset# stream data here
            if len(voltset) != len(time):
                 #pass
                print(len(voltset))
                print(len(time))
                print(1/0)
               # print("error for i on voltset shape : ", idx)
    #         #if i < 5:
           ...
           # print("voltset len:", len(voltset), "idx:",idx, "counter:", test_counter)
            voltset = None
            test_counter += 1
            #print(test_counter)
    #print(len(traces), "traces len")
    ...
    diff_features = []
    for i in range(len(traces_results)-1): #testing
