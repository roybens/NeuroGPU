...
params_file_path = '../run_volts_bbp_passive/params/params_bbp_passive.hdf5'
	...
	for key in sensitivity_dictionary.keys():
		...
		for i in range(len(curr_mat)):
			for j in range(len(curr_mat[i])):
				if curr_mat[i][j] > 1 or np.isnan(curr_mat[i][j]):
					curr_mat[i][j] = 1
...
if COMM.rank == 0:
	...
	opt_result_hdf5 = h5py.File(optimization_results_path+'opt_result_single_stim_without_sensitivity_passive.hdf5', 'w')
