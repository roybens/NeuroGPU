...
score_path = '../scores_mainen_full/'
optimization_results_path = './optimization_results_path/'
params_file_path = '../params.hdf5'
...
def construct_objective(score_mat, sensitivity_mat, param_select_vec=None, obj_comb_vec=[1, 1]):
	...
	# argmin of objective will be the optimal linear combination
		...
		if param_select_vec:
			...
			mean = np.mean(np.dot(x/float(np.linalg.norm(x)), selected_sensitivity_mat))
		else:
			mean = np.mean(np.dot(x/float(np.linalg.norm(x)), sensitivity_mat))
		if np.isnan(spearman):
			print("There is NAN")
		return np.dot([-spearman, -mean], obj_comb_vec)
	return obj
...
def optimize(stim_name_list, pin_score_dict, collapsed_sensitivity_dict):
	...
	obj = construct_objective(score_mat, sensitivity_mat, param_select_vec=[9, 10, 11], obj_comb_vec=[1, 1])
	
	return minimizeCompass(obj, bounds=bound, x0=initial_guess, deltainit = 1000, deltatol=1, paired=False)
...
if COMM.rank == 0:
	...
	opt_result_hdf5 = h5py.File(optimization_results_path+'multi_stim_with_sensitivity_mainen_full_1_1_0_40_stims_9_10_11.hdf5', 'w')
