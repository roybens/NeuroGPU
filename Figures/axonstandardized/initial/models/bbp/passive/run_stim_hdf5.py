...
params_file_path = './params/params.hdf5'
stims_file_path = './stims/stims.hdf5'
...
if COMM.Get_rank() == 0:
	...
	for params_name in params_name_list:
		if 'orig' in params_name:
			...
			for param_ind in range(n):
				jobs.append([params_name, param_ind, n])
			#print(params_name, " : ", np.shape(params_nwb.data[:]))
	...
	print('after', len(jobs), COMM.Get_rank())
else:
	jobs = None
# print('rank', COMM.rank, len(jobs))
# sys.exit(1)
...
for job in jobs:
	...
	# np.savetxt(volts_path + str(param_ind) +'.csv', volts_at_i, delimiter=',')
	...
	for params_name in params_name_list:
		...
		if 'orig' in params_name:
			...
			for param_ind in range(pin_set_size):
				volts[param_ind] = flattened_dict[(params_name, param_ind)]
				print("Processing ", name_to_write, str(param_ind)+"/"+str(pin_set_size))
			volts_hdf5.create_dataset(name_to_write, data=volts)
		elif 'pdx' in params_name:
			...
			for param_ind in range(pdx_set_size):
				volts[param_ind] = flattened_dict[(params_name, param_ind)]
				print("Processing ", name_to_write, str(param_ind)+"/"+str(pdx_set_size))
			volts_hdf5.create_dataset(name_to_write, data=volts)
	volts_hdf5.close()
