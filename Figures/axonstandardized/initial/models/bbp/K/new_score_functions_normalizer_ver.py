...
# each timepoints. 
def chi_square_normal(target, data, dt=0.1, stims=None, index=None):
...
def abs_cumsum_diff(target, data, dt=0.1, stims=None, index=None):    
...
def comp_rest_potential(target, data, dt=0.1, stims=None, index=None):
...
def comp_width(target, data, dt=0.1, stims=None, index=None):
...
def comp_width_avg(target, data, dt=0.1, stims=None, index=None): 
	global comp_width_dict
	if stims and index:
		stim_ind = stims + index
		if stim_ind in comp_width_dict:
...
def comp_height(target, data, dt=0.1, stims=None, index=None):
...
def comp_height_avg(target, data, dt=0.1, stims=None, index=None): 
	global comp_height_dict
	if stims and index:
		stim_ind = stims + index
		if stim_ind in comp_height_dict:
...
def traj_score_1(target, data, dt=0.1, stims=None, index=None):
...
def traj_score_2(target, data, dt=0.1, stims=None, index=None):
	global traj_score_dict
	if stims and index:
		stim_ind = stims + index
		if stim_ind in traj_score_dict:
...
def traj_score_3(target, data, dt=0.1, stims=None, index=None):
	global traj_score_dict
	if stims and index:
		stim_ind = stims + index
		if stim_ind in traj_score_dict:
...
def isi(target, data, dt=0.1, stims=None, index=None):
...
def rev_dot_product (target, data, dt=0.1, stims=None, index=None):
	def vectorSize(v):
...
def KL_divergence (target, data, dt=0.1, stims=None, index=None): 
	def shift(v):
...
def traj_score_single_peak(target, data, dt = 0.1, stims = None, index = None):
    X_size = 100
    Y_size = 100
...
def DTWDistance(target, data, dt = 0.1, stims = None, index = None):
