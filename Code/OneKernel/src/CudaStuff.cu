...
__device__ void runSimulation(HMat InMat, const MYFTYPE* __restrict__ ParamsM, MYFTYPE* ModelStates, MYFTYPE* V, Stim stim, Sim sim, MYFTYPE* VHotGlobal, MYFTYPE* SMemVHot, MYFTYPE* amps, MYDTYPE offset)
{
	...
	for (int i = 0; i<Nt; i++) {
		...
		for (int count = 1; count < NILP + 1; count++) {
			...
			if (PIdx[count] == stimLoc) {
				StimCurrent[count] = 100 * amps[i%WARPSIZE] / stimArea;
			}
		}
		for (int count = 1; count < NILP + 1; count++) {
...
__global__ void NeuroGPUKernel(Stim stim, MYFTYPE* ParamsM, MYFTYPE* ModelStates, Sim sim, HMat InMat, MYFTYPE *V, MYFTYPE* VHotGlobal, MYDTYPE CompDepth, MYDTYPE CompFDepth)
{
	...
	MYDTYPE offset = 0;
	runSimulation(InMat, ParamsM, ModelStates, V, stim, sim, VHotGlobal, SMemVHot, amps, offset);
}
...
//#ifndef NKIN_STATES
	...
	NeuroGPUKernel << <currKernelRun, blockDim, TotalSMem >> >(stim_d, d_modelParams, d_modelStates, sim_d, Mat_d, V_d, VHotsGlobal, CompDepth, CompFDepth); // RRR	
